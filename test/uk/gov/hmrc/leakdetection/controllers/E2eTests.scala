...
import uk.gov.hmrc.leakdetection.model.{PushDelete, PushUpdate, Report, RepositoryEvent}
...
class E2eTests
  ...
  given OFormat[WebhookResponse] = Json.format[WebhookResponse]
  given Timeout = Timeout(10.seconds)
  given Writes[PushUpdate] = pushUpdateWrites
  given Writes[PushDelete] = pushDeleteWrites
  given Writes[RepositoryEvent] = repositoryEventWrites
  ...
  val queueRepo: GithubRequestsQueueRepository =
    new GithubRequestsQueueRepository(Configuration(), mongoComponent):
  ...
  lazy val reportsRepo = ReportsRepository(mongoComponent)
